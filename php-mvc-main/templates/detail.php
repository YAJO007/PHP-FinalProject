...
<body class="bg-gradient-to-br from-purple-200 via-purple-300 to-purple-400 min-h-screen flex items-center justify-center p-6">
    ...
    <div class="bg-white border-2 border-black rounded-[24px]
        ...
        <div class="p-4 bg-purple-100 grid md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                ...
                $has_checked_in = $is_registered ? isUserCheckedIn($user_id, (int)$event['eid']) : false;
                ?>
                ...
                <?php if ($is_logged_in && $is_registered && !$is_owner): ?>
                    <div class="bg-white border-2 border-black rounded-lg p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                            ...
                            <?php if ($has_checked_in): ?>
                                <div class="flex-1">
                                    <div class="bg-blue-100 border-2 border-blue-500 rounded-lg p-3">
                                        <p class="font-bold text-blue-800 mb-2">เข้าร่วมแล้ว!</p>
                                        <p class="text-sm text-blue-700">คุณได้เข้าร่วมกิจกรรมนี้เรียบร้อยแล้ว</p>
                                    </div>
                                </div>
                            <?php elseif ($registration_status === 'Approved'): ?>
                                <div class="flex-1">
                                    <div class="bg-green-100 border-2 border-green-500 rounded-lg p-3">
                                        ...
                                        <p class="text-sm text-green-700">คุณได้รับการอนุมัติแล้ว สามารถเข้าร่วมกิจกรรมได้</p>
                                    </div>
                                </div>
                        ...
                        <?php if ($registration_status === 'Approved' && !$has_checked_in): ?>
                            <!-- OTP Request Button -->
                            <div class="mt-4">
                                <button onclick="generateParticipantOTP(<?= (int)$event['eid'] ?>)" 
                                        class="w-full px-4 py-3 bg-blue-600 text-white border-2 border-black rounded-lg font-bold
                                               shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]
                                               hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all
                                               flex items-center justify-center gap-2">
                                    <i class="fa-solid fa-key"></i>
                                    ขอรหัส OTP เข้าร่วมกิจกรรม
                                </button>
                                
                                <!-- OTP Display (Hidden by default) -->
                                <div id="participantOtpDisplay" class="hidden mt-3 p-3 bg-yellow-100 border-2 border-yellow-400 rounded-lg">
                                    <p class="font-bold text-yellow-800 mb-1">รหัส OTP ของคุณ:</p>
                                    <div class="flex items-center gap-3">
                                        <span id="participantOtpCode" class="text-2xl font-black text-green-600 tracking-widest">------</span>
                                        <span id="participantOtpTimer" class="text-sm text-gray-600">10:00</span>
                                    </div>
                                    <p class="text-xs text-gray-600 mt-2">แสดงรหัสนี้ให้ผู้จัดงานเพื่อยืนยันการเข้าร่วม</p>
                                    
                                    <div class="mt-3 bg-blue-50 border-2 border-blue-200 rounded-lg p-2">
                                        <p class="text-sm text-blue-800 font-bold mb-1">
                                            <i class="fa-solid fa-info-circle"></i> วิธีใช้งาน:
                                        </p>
                                        <ol class="text-xs text-gray-700 list-decimal list-inside space-y-1">
                                            <li>แสดงรหัส OTP นี้ให้ผู้จัดงาน</li>
                                            <li>ผู้จัดงานจะกรอกรหัสเพื่อยืนยันการเข้าร่วม</li>
                                            <li>รหัสใช้ได้ 10 นาทีเท่านั้น</li>
                                        </ol>
                                    </div>
                                </div>
                            </div>
    ...
    <?php if (isset($_GET['checkin']) && $_GET['checkin'] === 'success' && isset($_SESSION['checkin_success'])): ?>
        <div class="fixed top-4 right-4 bg-green-100 border-2 border-green-500 rounded-lg p-4 shadow-lg z-50">
            <p class="font-bold text-green-800">✅ เข้าร่วมกิจกรรมสำเร็จ!</p>
        </div>
...
function generateParticipantOTP(eid) {
    fetch('generate_event_otp?eid=' + eid + '&ajax=1&t=' + new Date().getTime())
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.error === 'Already checked in' ? 'คุณได้เข้าร่วมกิจกรรมนี้แล้ว' : 'ไม่สามารถสร้าง OTP ได้: ' + (data.error || 'เกิดข้อผิดพลาด'));
            }
        })
        .catch(() => {
